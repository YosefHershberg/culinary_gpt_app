...
import useAddImageIngredients from "@/hooks/componentHooks/useAddImageIngredients"
  ...
  const renderIngredientsList = () => (
    <div className="flex flex-col items-center mx-3">
      ...
      <div className="flex items-start w-full my-3 ml-5">
        <label htmlFor="all" className="flex items-center gap-3">
          <Checkbox
            id="all"
            onCheckedChange={(e) =>
              e ? handleCheckAllIngredients() : handleRemoveAllIngredients()
            }
            checked={ingredientResults?.every(
      ...
      <DialogFooter>
        <DialogClose asChild>
          <Button
            disabled={ingredientResults?.every(
              (ingredient) => !ingredient.checked
            )}
            onClick={() => {
              handleAddIngredientsFromImage();
  ...
  const renderLoadingState = () => (
    <div className="h-full w-full flex flex-col">
      <DialogTitle className="text-center">
        Checking the image for ingredients...
      </DialogTitle>
      <DialogDescription className="text-center mt-3">
